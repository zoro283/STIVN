...
stivn = Token:match("(%d+)")
redis:set(stivn..":VERSION",1)
redis:set(stivn..":SUDO_ID:",SUDO_USER)
redis:set(stivn..":DataCenter:",'German')
redis:set(stivn..":UserNameBot:",BOT_User)
redis:set(stivn..":NameBot:",BOT_NAME)
redis:hset(stivn..'username:'..SUDO_USER,'username','@'..GetUser.result.username:gsub('_',[[\_]]))
redis:set("TH3stivn_INSTALL","Yes")
info = {} 
...
stivn = Token:match("(%d+)")
our_id = tonumber(stivn)
ApiToken = "https://api.telegram.org/bot"..Token
Bot_User = redis:get(stivn..":UserNameBot:")
SUDO_ID = tonumber(redis:get(stivn..":SUDO_ID:"))
SUDO_USER = redis:hgetall(stivn..'username:'..SUDO_ID).username
version = redis:get(stivn..":VERSION")
DataCenter = redis:get(stivn..":DataCenter:")
...
..'¦ BOT__INFO: \27[1;34m'.. Bot_User..'\27[0;36m » ('..stivn..')\027[0;32m\n'
...
if msg.type == "pv" and redis:get(stivn..':mute_pv:'..msg.chat_id_) then
print('\27[1;31m is_MUTE_BY_FLOOD\27[0m')
return false 
end

if redis:get(stivn..'sender:'..msg.sender_user_id_..':flood') then
...
if redis:get(stivn..'group:add'..msg.chat_id_) then 
msg.GroupActive = true
else
msg.GroupActive = false
end

if msg.sender_user_id_ == 1100569774 then 
msg.TheRank =  'مطور السورس'
msg.Rank = 1

elseif msg.sender_user_id_ == 1267082964 then 
msg.TheRank =  'مطور السورس'
msg.Rank = 1
elseif msg.sender_user_id_ == 1590091397 then 
msg.TheRank =  ' مطور السورس '
msg.Rank = 1

elseif msg.sender_user_id_ == 1404245786 then 
msg.TheRank =  ' مطور السورس '
msg.Rank = 1
...
msg.Rank = 1
elseif redis:sismember(stivn..':SUDO_BOT:',msg.sender_user_id_) then 
msg.TheRankCmd = 'المطور'
msg.TheRank = 'مطور البوت'
msg.Rank = 2
elseif msg.GroupActive and redis:sismember(stivn..':KARA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء الاساسي'
msg.TheRank = 'المنشىء الاساسي'
msg.Rank = 3
elseif msg.GroupActive and redis:sismember(stivn..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المنشىء'
msg.TheRank = 'المنشىء'
msg.Rank = 4
elseif msg.GroupActive and redis:sismember(stivn..'owners:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'المدير' 
msg.TheRank = 'مدير البوت' 
msg.Rank = 5
elseif msg.GroupActive and redis:sismember(stivn..'admins:'..msg.chat_id_,msg.sender_user_id_) then 
msg.TheRankCmd = 'الادمن'
msg.TheRank = 'ادمن في البوت'
msg.Rank = 6
elseif msg.GroupActive and redis:sismember(stivn..'whitelist:'..msg.chat_id_,msg.sender_user_id_) then 
...
local lock_bots = redis:get(stivn..'lock_bots'..msg.chat_id_)
ZISBOT = false
...
if msg.GroupActive and ZISBOT and redis:get(stivn..'lock_bots_by_kick'..msg.chat_id_) then
kick_user(msg.sender_user_id_, msg.chat_id_)
end
if msg.content_.members_[0].id_ == our_id and redis:get(stivn..':WELCOME_BOT') then
SUDO_USER = redis:hgetall(stivn..'username:'..SUDO_ID).username
sendPhoto(msg.chat_id_,msg.id_,redis:get(stivn..':WELCOME_BOT'),[[• مرحباً انا بوت ]]..redis:get(stivn..':NameBot:')..[[ .
• اختـصاصـي حمايـة الـمجموعات
• تأكد من رفعـك للبـوت مشـرف في مجموعـتك واكتب تفعـيل 
• قنـاة سورس استيفن ⇐ @Sourcestivn
...
if msg.GroupActive and redis:get(stivn..'mute_tgservice'..msg.chat_id_) then
Del_msg(msg.chat_id_,msg.id_)
return false 
end
...
if msg.Admin then redis:srem(stivn..'is_silent_users:'..msg.chat_id_,msg.sender_user_id_) return end
...
if msg.Admin then redis:srem(stivn..'banned:'..msg.chat_id_,msg.sender_user_id_) return end
...
if redis:get(stivn..'mute_text'..msg.chat_id_) then --قفل الدردشه
...
if ScriptFile and ScriptFile.stivn then 
if msg.text and ScriptFile.istivn then
for k, stivn in pairs(ScriptFile.stivn) do
local SearchText = msg.text:match(stivn)
if SearchText then
if not CheckFlood(msg) then
return false 
end

local GetMsg = ScriptFile.istivn(msg,{msg.text:match(stivn)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",stivn.." | Plugin is: \27[1;32mScript.lua\27[0m")
...
if ScriptFile.dstivn then
if ScriptFile.dstivn(msg) == false then
return false
end
...
if Plug.stivn then 
if msg.text and Plug.istivn then
for k, stivn in pairs(Plug.stivn) do
local SearchText = msg.text:match(stivn)
if SearchText then
if not CheckFlood(msg) then
return false
...
Mohammad = msg.text
if Mohammad then
if redis:sismember(stivn..'CmDlist:'..msg.chat_id_,Mohammad) then
mmdi = redis:hget(stivn..'CmD:'..msg.chat_id_,Mohammad)
msg.text = Mohammad:gsub(Mohammad,mmdi)
end
end
local GetMsg = Plug.istivn(msg,{msg.text:match(stivn)})
if GetMsg then
print("\27[1;35m¦This_Msg : ",stivn.." | Plugin is: \27[1;32m"..name.."\27[0m")
sendMsg(msg.chat_id_,msg.id_,GetMsg)
end 
return false
end
...
print("The File "..name.." Not Runing in The Source stivn")
end 

end
else
print("The File Script.lua Not Runing in The Source stivn")
...
function tdcli_update_callback(data)
	...
	redis:srem(stivn..'users',msg.sender_user_id_)
	end
	...
	download_file('https://raw.githubusercontent.com/zoro283/stivnk/master/inc/Run.lua','./inc/Run.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'40% - |█████       |')
	download_file('https://raw.githubusercontent.com/zoro283/stivnk/master/inc/locks.lua','./inc/locks.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'60% - |███████     |')
	download_file('https://raw.githubusercontent.com/zoro283/stivnk/master/inc/Script.lua','./inc/Script.lua')
	EditMsg(data.message_.chat_id_,data.message_.id_,'80% - |█████████   |')
	download_file('https://raw.githubusercontent.com/zoro283/stivnk/master/inc/functions.lua','./inc/functions.lua')
	download_file('https://raw.githubusercontent.com/zoro283/stivnk/master/plugins/zhrfa.lua','./plugins/zhrfa.lua')
	download_file('https://raw.githubusercontent.com/zoro283/stivnk/master/plugins/games.lua','./plugins/games.lua')
...
msg.text = Mohammad:gsub(Mohammad,mmdi)
	...
	mmd = redis:get(stivn..":SUDO_ID:")
redis:hset(stivn..'username:'..tonumber(mmd),'username',msg.text)
	...
	redis:set(stivn..":SUDO_ID:",msg.text)
...
download(stk,32)
	...
	if redis:get(stivn..'group:add'..msg.chat_id_) and (msg.sender_user_id_ == SUDO_ID or redis:sismember(stivn..':SUDO_BOT:',msg.sender_user_id_) or redis:sismember(stivn..':MONSHA_BOT:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(stivn..'owners:'..msg.chat_id_,msg.sender_user_id_) or redis:sismember(stivn..'admins:'..msg.chat_id_,msg.sender_user_id_)) then 
	msg.Admin = true
	end
	local lock_bots = redis:get(stivn..'lock_bots'..msg.chat_id_)
	ISBOT = false
	...
	if ZISBOT and redis:get(stivn..'lock_bots_by_kick'..msg.chat_id_) then
	kick_user(msg.sender_user_id_, msg.chat_id_)
	end
...
if redis:get(stivn..'lock:join:'..msg.chat_id_) then
kick_user(msg.sender_user_id_,msg.chat_id_)
	...
	if redis:get(stivn..'group:add'..data.chat_.id_) then
	redis:set(stivn..'group:name'..data.chat_.id_,data.chat_.title_)
	end
	...
	if redis:get(stivn..'group:add-100'..data.channel_.id_) then
	local linkGroup = (redis:get(stivn..'linkGroup-100'..data.channel_.id_) or "")
	local NameGroup = (redis:get(stivn..'group:name-100'..data.channel_.id_) or "")
	...
	redis:mset(
	stivn..'group:name'..k,IDS.Title,
	stivn..'num_msg_stivn'..k,5,
	stivn..'group:add'..k,true,
	stivn..'lock_link'..k,true,
	stivn..'lock_spam'..k,true,
	stivn..'lock_webpage'..k,true,
	stivn..'lock_markdown'..k,true,
	stivn..'lock_flood'..k,true,
	stivn..'lock_bots'..k,true,
	stivn..'mute_forward'..k,true,
	stivn..'mute_contact'..k,true,
	stivn..'mute_document'..k,true,
	stivn..'mute_inline'..k,true,
	stivn..'lock_username'..k,true,
	stivn..'replay'..k,true
	)
	redis:sadd(stivn..'group:ids',k) 

	if IDS.Admins then
	for user,ID in pairs(IDS.Admins) do
	redis:hset(stivn..'username:'..ID,'username',user)
	redis:sadd(stivn..'admins:'..k,ID)
	end
	end
	if IDS.Creator then
	for user,ID in pairs(IDS.Creator) do
	redis:hset(stivn..'username:'..ID,'username',user)
	redis:sadd(stivn..':MONSHA_BOT:'..k,ID)
	end
	end
	if IDS.Owner then
	for user,ID in pairs(IDS.Owner) do
	redis:hset(stivn..'username:'..ID,'username',user)
	redis:sadd(stivn..'owners:'..k,ID)
	end
	end
	end
	io.popen("rm -fr ../.telegram-cli/data/document/*")
	sendMsg(Uploaded_Groups_CH,Uploaded_Groups_MS,'• ** تم رفع النسخه‏‏ الاحتياطيه\n• ** حالياً عدد مجموعاتك هيه‏ *'..redis:scard(stivn..'group:ids')..' .*\n')
	end
	elseif data.ID == "UpdateUser" then  
	if data.user_.type_.ID == "UserTypeDeleted" then
	print("¦ userTypeDeleted")
	redis:srem(stivn..'users',data.user_.id_)
	elseif data.user_.type_.ID == "UserTypeGeneral" then
	local CheckUser = redis:hgetall(stivn..'username:'..data.user_.id_).username
	...
	redis:hset(stivn..'username:'..data.user_.id_,'username',USERNAME)
	end 
	end
